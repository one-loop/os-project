...
static int send_all(int fd, const void *buf, size_t len) {
    // walk across the buffer until every byte in this fixed-size frame is sent.
...
static int recv_all(int fd, void *buf, size_t len) {
    // read repeatedly until one full fixed-size frame is received.
    char *p = (char *)buf;
    size_t got = 0;
...
static int connect_to_server(void) {
    ...
    // clear the struct so any unused fields stay deterministic.
...
int main(void) {
    ...
    while (1) {
        ...
        if (command[0] == '\0') {
            // empty input should not generate a request/response round trip.
            continue;
        }
    ...
    // always close the socket before process exit to release os resources.
    close(network_socket);
    return 0;
}
